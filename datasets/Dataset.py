...
from datasets.Resize import resize, ResizeMode
from datasets.util.BoundingBox import encode_bbox_as_mask, get_bbox_from_segmentation_mask
from datasets.util.DistanceTransform import signed_distance_transform, unsigned_distance_transform
...
class FileListDataset(AbstractDataset):
  ...
  def _batch(self, tfdata, batch_size):
    if batch_size > 1:
      # the api was changed so that it woul produce smaller batches for the last step, which might in some cases cause problems
