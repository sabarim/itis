...
from datasets.util.Util import resize_image, random_crop_image, resize_coords
...
def random_resize_and_crop(tensors, size):
  ...
  if len(size) == 2:
    ...
    crop_size = [size, size]
  tensors, _ = resize_random_scale_with_min_size(tensors, min_size=crop_size)
...
def fixed_resize_and_crop(tensors, size):
  ...
  tensors, _ = object_crop_fixed_offset(tensors, crop_size)
...
def resize_random_scale_with_min_size(tensors, min_size, min_scale=0.7, max_scale=1.3, scaled_size=None):
  ...
  return tensors_out, scaled_size
...
def centre_warp(tensors, size, tform):
  keys_to_crop = [DataKeys.IMAGES, DataKeys.SEGMENTATION_LABELS, DataKeys.SEGMENTATION_LABELS_ORIGINAL_SIZE,
                  DataKeys.BBOX_GUIDANCE, DataKeys.RAW_SEGMENTATION_LABELS, DataKeys.NEG_CLICKS, DataKeys.POS_CLICKS,
                  DataKeys.BBOX_GUIDANCE]
  tensors_resized = tensors.copy()

  for key in keys_to_crop:
    if key in tensors:
      # tensors_resized[key] = transform(tensors[key], tform)
      tensors_resized[key] = tf.py_func(transform, [tensors[key], tform, size, key], tf.float32, name="transform")
      tensors_resized[key].set_shape([size[0], size[1], tensors[key].get_shape()[2]])

  # if DataKeys.SEGMENTATION_LABELS in tensors:
  #   tensors_resized[DataKeys.SEGMENTATION_LABELS_ORIGINAL_SIZE] = \
  #     tensors[DataKeys.SEGMENTATION_LABELS]
  return tensors_resized


def transform(tensor, tform, size, key):
  tensor = tensor.copy()
  key_str = key.decode('utf-8')
  if key_str != DataKeys.IMAGES:
    result = np.where(tensor == 255, 2, tensor)
    result = cv2.warpAffine(result, tform[0:2, :], (size[0], size[1]))
    # print(key, np.unique(result), np.unique(tensor))
    result = np.expand_dims(result, axis=2)
    result[result == 2] = 255
  else:
    result = cv2.warpAffine(tensor, tform[0:2, :], (size[0], size[1]))
