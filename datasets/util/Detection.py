...
def get_rpn_anchor_input(boxes, boxes_ids, is_crowd, H, W):
  """
  Args:
...
def add_rpn_data(return_dict):
  ...
  fm_labels, fm_boxes, invalid_bgfg = tf.py_func(get_rpn_anchor_input, [gt_boxes_xyxy, return_dict[DataKeys.IDS],
                                                 return_dict[DataKeys.IS_CROWD], img_shape[0], img_shape[1]],
...
def load_instance_seg_annotation_np(ann_filename, n_max_detections, class_ids_with_instances, id_divisor):
  ann_filename = ann_filename.decode("utf-8")
  ann = np.array(Image.open(ann_filename))
  obj_ids = np.unique(ann)
  obj_ids = np.array([x for x in obj_ids if x // id_divisor in class_ids_with_instances], dtype="int32")
