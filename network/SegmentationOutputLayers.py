...
from network.Util import prepare_input, get_activation, apply_dropout, conv2d, conv2d_dilated, bootstrapped_ce_loss, \
  class_balanced_ce_loss
from datasets import DataKeys
from core.Measures import compute_measures_for_binary_segmentation_tf
...
class SegmentationSoftmax(Layer):
  ...
  def __init__(self, name, inputs, dataset, network_input_dict, tower_setup, resize_targets=False,
    ...
    if (not tower_setup.is_training) and batch_size == 1 and \
            DataKeys.SEGMENTATION_LABELS_ORIGINAL_SIZE in network_input_dict:
