...
from core import Extractions, Measures
...
from datasets.PascalVOC.PascalVOC_masked_dios import PREV_MASK
from datasets.util.DistanceTransform import get_distance_transform
from datasets.util.Util import decodeMask
from network.Network import Network
from scripts.Datasets.EvalDAVIS import EvalDAVISDataset
from scripts.Datasets.EvalGrabcut import EvalGrabcutDataset
from scripts.Datasets.EvalKITTI import EvalKittiDataset
from scripts.Datasets.EvalOSVOSWorst import OSVOSWorst
from scripts.Datasets.EvalPascalMasked import EvalPascalMaskedDataset, CURRENT_CLICK
...
def load_eval_dataset(config, subset):
  ...
  elif name == "kitti_masked_dios":
    return EvalKittiDataset(config, subset)
  else:
  ...
  def eval(self):
    for round in range(self.recursive_rounds):
      ...
      while n_examples_processed < n_examples_per_epoch:
        timer = Timer()
        extraction_keys = [Extractions.SEGMENTATION_MASK_INPUT_SIZE, Extractions.SEGMENTATION_MASK_ORIGINAL_SIZE,
