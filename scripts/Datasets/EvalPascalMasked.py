...
from datasets.PascalVOC.PascalVOC_masked_dios import PascalVOCMaskedDiosDataset, PREV_MASK, PREV_NEG_CLICKS, \
  PREV_POS_CLICKS
...
from datasets.util.Util import decodeMask, visualise_clicks
...
def generate_click_for_correction(label, prediction, previous_clicks, void_label, n_clicks=1, d_step=5,
                                  random_cluster=False, random_sampling=False):
  # minimum distance from margin: used only if random_sampling is set to true.
  threshold = 0
  ...
  if random_cluster:
    ...
    largest_cluster = np.where(cluster_counts == largest_cluster)[0][0]
    largest_cluster += 1
  else:
  ...
  for i in range(n_clicks):
    ...
    if np.max(dt) > threshold:
      # get points that are farthest from the object boundary.
      # farthest_pts = np.where(dt > np.max(dt) / 2.0)
      if random_sampling:
        pts = np.where(dt > threshold)
        farthest_pts = random.sample(list(zip(pts[0], pts[1])), 1)
      else:
...
class EvalPascalMaskedDataset(PascalVOCMaskedDiosDataset):
  def __init__(self, config, subset):
    ...
    self.random_cluster = config.bool("use_random_cluster", False)
    self.random_sampling = config.bool("use_random_sampling", False)
  ...
  def dios_distance_transform(self, label, raw_label, ignore_classes, img_filenames):
    ...
    clicks = generate_click_for_correction(label, prediction, neg_clicks + pos_clicks, void_label=255,
                                           random_cluster=self.random_cluster, random_sampling=self.random_sampling)
  ...
  def use_segmentation_mask(self, res):
    ...
    prediction = extractions[Extractions.SEGMENTATION_MASK_INPUT_SIZE][0][0] \
      if Extractions.SEGMENTATION_MASK_INPUT_SIZE in extractions \
      else extractions[Extractions.SEGMENTATION_MASK_ORIGINAL_SIZE][0][0]
    ...
    if self.save_images:
      ...
      fn = main_folder + filename.split(".")[0].split("/")[-1] + "_" + str(filename.split(":")[-1]) + \
           str(num_clicks) + ".png"
      neg_click_fn = main_folder + filename.split(".")[0].split("/")[-1] + "_" + str(filename.split(":")[-1]) + \
           str(num_clicks) + "_neg.png"
      pos_click_fn = main_folder + filename.split(".")[0].split("/")[-1] + "_" + str(filename.split(":")[-1]) + \
                     str(num_clicks) + "_pos.png"
      udt_fn = main_folder + filename.split(".")[0].split("/")[-1] + "_" + str(filename.split(":")[-1]) + \
                     str(num_clicks) + "_udt.png"
      ...
      imsave(fn, img[0])
      imsave(neg_click_fn, inputs[:, :, 3])
      imsave(pos_click_fn, inputs[:, :, 4])
      imsave(udt_fn, inputs[:, :, 5])
